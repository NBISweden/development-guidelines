...
Things to be aware of when writing code        2
How we use GitHub        5
How we use Git        5
General stuff about working with Git        7
How we do code reviews        7
General stuff about code reviews        9
...
For example, function names might be a verb or a question: get_gene_name(), find_downstream_feature(), is_circular(), or has_multiple_flurbs().
   ...
   * Google has a good set of best practices for different languages which can be a good jump off point https://github.com/google/styleguide
   * For Perl: Perl Best Practices (O’Reilly book) http://shop.oreilly.com/product/9780596001735.do
...
Sensitive data includes like passwords, usernames, server names, and data protected by law.
               ...
               * Do not ever put sensitive data in files that are pushed to GitHub or made public in any other way. It’s worth repeating![a][b] Always try to place your sensitive data outside directories that are pushed to GitHub (to avoid mistakes). Design your code so sensitive data are read from separate folders or files not included in the git repo.[c][d]
               ...
               * Robustness test
               * Test running of the code before releasing: blind test[e][f][g][h].
...
Commit often, possibly several times a day. It’s easier to roll back a small commit than to roll back large commits. This also makes the code easier to review (see below) as each commit carries its own commit message. Remember to push the commits to GitHub every once in awhile too.
                           * Write a helpful commit message[i][j][k] with each commit that describes what the changes are and possibly even why they were necessary.
...
Learn how to “cherry pick” chunks of changed files to do multiple separate commits of unrelated things. This is done using “git commit -p ...”.
                           ...
                           * If a “live” checkout of the repository needs to exist somewhere, for example to run a public web service, then
                              ...
                              * A live service with active users should run a stable release from the master branch.[l]
...
A reviewer may be found
                                 ...
                                 1. If needed, the author gives the reviewer(s) some background on the project, and what the code under review is supposed to do etc. Having a fixed group of reviewers for a project would minimize the need for this step.
This may be done in a face-to-face meeting, on Slack, or in any other way that is convenient. It’s also possible for the author to leave comments on GitHub (in addition to the meta-comments that the commit messages themselves already provide)
                                    1. With the pull request.
                                    2. On separate commits (?).
                                    3. On separate lines in the commits.
                                    1. If there’s more than one reviewer, one of the reviewers is designated as the “main” reviewer. This reviewer will later do one extra thing (see below).
                                    2. The reviewer(s) looks at the code, specifically at the bits of code that the specified pull request is about.
                                    3. The reviewer(s) leaves comments and/or questions in the code by clicking individual lines in the web viewer. Note that these comments are public.
                                    4. The reviewer(s) leaves a summary of their review by clicking “Review changes” and submits it as feedback
                                    1. without explicitly approving the pull request (it’s just feedback)
                                    2. explicitly approving the pull request (it all look good), or
                                    3. explicitly rejecting the pull request (there’s something that needs to be discussed and/or fixed).
                                    1. If no reviews are rejecting the pull request, the designated “main” reviewer will merge the pull request and delete the feature (or whatever) branch. Note: this is the reviewer’s job, not the author’s job. The code review process ends here.
                                    2. If there are things that need to be modified, further commits to the same feature branch may be necessary. These commits are automatically added to the existing pull request.
                                    3. The author asks the reviewer(s) to have a further look at the new changes. The process continues from step 7.
...
The following is adapted from https://github.com/thoughtbot/guides/tree/master/code-review
...
Elixir best practice
                                    * https://github.com/SoftDev4LS
                                    * https://drive.google.com/open?id=0Bw_p-HKWUjHoNG0xLW96VjQ5SjA
...
[a]You could add something like: "Try not to put sensitive data in directories that are pushed to GitHub - link the sensitive data in a way that it can not be pushed by mistake"
[b]can you add a suggestion to the text?
[c]lite så?
[d]I've added text further down to the same effect (where it talks about "placeholders"). Would this be enough?
[e]Need more info here. Also, I don't know what a "blind test" is.
[f]Äsch det är en anonym användare som skrivit det? Vem kan det vara?
[g]Whoever it is, we might need a separate "testing" section. I'm not qualified to write it though.
[h]googling about blind test and software do not give so much. Color blind test?...
[i]Add e.g. the seven rules for a great commit message?
...
[j]I'll try to incorporate something like this in the document.
[k]yes good idea!
[l]I added this. I hope it makes sense. In any case, this is the ideal situation.